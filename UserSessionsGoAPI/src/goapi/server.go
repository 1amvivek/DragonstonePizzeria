...
type (
	...
	User struct {
		...
		LogsSerialNumber string `json: "id"`
...
func updateHelper(server_val string, user User) {
	...
	if s != nil {
		...
		newUserList := fmt.Sprint(current.Users +"," + user.Users)
		err2 := c.Update(bson.M{"serialnumber": user.SerialNumber}, bson.M{"$set": bson.M{"name": current.Name,"owner": current.Owner,"users": newUserList,"cartserialnumber": current.CartSerialNumber,"logsserialnumber": current.LogsSerialNumber, "clock": (current.Clock + 1)}})
			...
			for index, object := range objects {
				if object.Clock != result.Clock {
					if connections[index] != nil {
						...
						err2 := c.Update(bson.M{"serialnumber": result.SerialNumber}, bson.M{"$set": bson.M{"name": result.Name,"owner": result.Owner,"users": result.Users,"cartserialnumber": result.CartSerialNumber,"logsserialnumber": result.LogsSerialNumber, "clock": (result.Clock)}})
						if err2 != nil {
							fmt.Println("Some Random error")
							return
						}
...
func postHandler(formatter *render.Render) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		Jsonvalue, err5 := json.Marshal(user1)
